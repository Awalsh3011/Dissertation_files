# -*- coding: utf-8 -*-
"""Dissertation code - Random Under sampling.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1TcPtajvnttoY42dUaKbxXWz-Wsf0SD_9
"""
...
Need to use stratified K fold cause the data is imbalanced

Need to try random oversamplingROS, Random Under Sampling RUS and ensemble oversampling
"""
...
print('Resampled dataset shape {}'.format(Counter(y_under)))
...
grid_search = GridSearchCV(estimator = KNeighborsClassifier(), param_grid=knn_params, cv= 10, n_jobs=-1, verbose= 3 , scoring = "roc_auc", return_train_score=True)
...
for k in k_range:
    ...
    scores = cross_val_score(knn, x_under, y_under, cv=10, scoring='accuracy')
...
log = LogisticRegression(C=0.5, penalty='l2', solver='newton-cg')
log.fit(x_under, y_under)
y_predict = log.predict(x_test)
log_accuracy = metrics.accuracy_score(y_test, y_predict)
print('Accuracy of Logistic Regression = {} '.format(log_accuracy))
...
randomForest = RandomForestClassifier(criterion='entropy', max_depth=8, max_features='log2', min_samples_leaf=1, min_samples_split=10)
...
tree = DecisionTreeClassifier(criterion='entropy', max_depth=5, min_samples_leaf=15)
tree.fit(x_under, y_under)
y_predict = tree.predict(x_test)
tree_accuracy = metrics.accuracy_score(y_test, y_predict)
print('Accuracy of Decision Tree = {} '.format(tree_accuracy))
...
print(classification_report(y_test, y_predict))
cm_d = confusion_matrix(y_test, y_predict)
print(cm_d)
...
tree =  DecisionTreeClassifier(criterion='gini', max_depth=5, min_samples_leaf=15,min_samples_split= 6)
tree.fit(x_under, y_under)
...
model = XGBClassifier(max_depth=5, max_features='sqrt', n_estimators=40, subsample=0.8)
model.fit(x_under, y_under)
...
grid_search = abm_grid.fit(x_under, y_under)
print('Best Parameters: ', grid_search.best_params_)
print('Best estimators: ', grid_search.best_estimator_)
...
print(cm_a)
plt.figure(figsize=(7,5))
sns.heatmap(cm_a, annot=True, fmt=".0f", linewidths=3, square=True, cmap='Blues', color="#cd1076")
plt.ylabel('actual label')
plt.xlabel('predicted label')
plt.show()
...
grid_search = svm_grid.fit(x_under, y_under)
print('Best Parameters: ', grid_search.best_params_)
print('Best estimators: ', grid_search.best_estimator_)
